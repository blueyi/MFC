...
void CCaptionButton::ChangeButtonState()
{
	m_bBtnChecked ? m_bBtnChecked = FALSE : m_bBtnChecked = TRUE;
		// redraw the NC area
		::RedrawWindow(m_hWndCap, m_rc, NULL, RDW_INVALIDATE | RDW_FRAME);
	::SendMessage(m_hWndCap, WMU_HITCAPBUTTON , (WPARAM)m_uiBtnCount, (LPARAM)m_bBtnChecked);
}
...
return ::DefWindowProc(m_hWndCap, WM_NCACTIVATE, bActive, 0L);
}
...
BOOL CCaptionButton::HookDlg()
{
	...
	if(!(m_pWndProcOld = (WNDPROC)::SetWindowLong(CCaptionButton::m_hWndCap , GWL_WNDPROC, (DWORD)CCaptionButton::HookedWindowProc)))
		return FALSE;
