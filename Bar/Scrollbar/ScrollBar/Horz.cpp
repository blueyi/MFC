...
BOOL CHorz::OnInitDialog() 
{
	...
	// red
	VERIFY(m_HorizontalScrollBar1.CreateFromStatic(
		SBS_HORZ|WS_CHILD|SS_LEFT|SS_NOTIFY|WS_VISIBLE,
		this, IDC_SCROLL1_RECT_HORZ, IDC_SCROLL1_HORZ));

	m_HorizontalScrollBar1.SetScrollRange(0, 255);
	m_HorizontalScrollBar1.SetThumbColor(RGB(255,0,0));
	m_HorizontalScrollBar1.EnableThumbGripper(m_bThumbGripper);
	GetDlgItem(IDC_RED_HORZ)->SetWindowText(_T("0"));
	
	// green
	VERIFY(m_HorizontalScrollBar2.CreateFromStatic(
		SBS_HORZ|WS_CHILD|SS_LEFT|SS_NOTIFY|WS_VISIBLE,
		this, IDC_SCROLL2_RECT_HORZ, IDC_SCROLL2_HORZ));

	m_HorizontalScrollBar2.SetScrollRange(0, 255);
	m_HorizontalScrollBar2.SetThumbColor(RGB(0,172,89));
	m_HorizontalScrollBar2.EnableThumbGripper(m_bThumbGripper);
	GetDlgItem(IDC_GREEN_HORZ)->SetWindowText(_T("0"));
	
	// blue
	VERIFY(m_HorizontalScrollBar3.CreateFromStatic(
		SBS_HORZ|WS_CHILD|SS_LEFT|SS_NOTIFY|WS_VISIBLE,
		this, IDC_SCROLL3_RECT_HORZ, IDC_SCROLL3_HORZ));

	m_HorizontalScrollBar3.SetScrollRange(0, 255);
	m_HorizontalScrollBar3.SetThumbColor(RGB(0,0,255));
	m_HorizontalScrollBar3.EnableThumbGripper(m_bThumbGripper);
	GetDlgItem(IDC_BLUE_HORZ)->SetWindowText(_T("0"));

	// saturation
	VERIFY(m_HorizontalScrollBar4.CreateFromStatic(
		SBS_HORZ|WS_CHILD|SS_LEFT|SS_NOTIFY|WS_VISIBLE,
		this, IDC_SCROLL4_RECT_HORZ, IDC_SCROLL4_HORZ));

	m_HorizontalScrollBar4.SetScrollRange(0, 100);
	m_HorizontalScrollBar4.EnableThumbGripper(m_bThumbGripper);
	GetDlgItem(IDC_SATURATION_HORZ)->SetWindowText(_T("0"));

	// luminance
	VERIFY(m_HorizontalScrollBar5.CreateFromStatic(
		SBS_HORZ|WS_CHILD|SS_LEFT|SS_NOTIFY|WS_VISIBLE,
		this, IDC_SCROLL5_RECT_HORZ, IDC_SCROLL5_HORZ));

	m_HorizontalScrollBar5.SetScrollRange(0, 100);
	m_HorizontalScrollBar5.EnableThumbGripper(m_bThumbGripper);
