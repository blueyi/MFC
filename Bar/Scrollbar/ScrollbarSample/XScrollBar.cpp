...
//=============================================================================
{
	...
	m_ArrowHotColor        = RGB(249,189,73);
	if (IsVista())
		m_ArrowHotColor    = RGB(120,198,249);
}
...
//=============================================================================
{
	...
	if (m_bmpUpLeftArrow2.GetSafeHandle())
		m_bmpUpLeftArrow2.DeleteObject();

	if (m_bmpDownRightArrow.GetSafeHandle())
		m_bmpDownRightArrow.DeleteObject();

	if (m_bmpDownRightArrow2.GetSafeHandle())
		m_bmpDownRightArrow2.DeleteObject();
...
BOOL CXScrollBar::CreateFromWindow(DWORD dwStyle,
								   CWnd* pParentWnd,
								   HWND hStatic,
								   HWND hStaticLeftArrow,
								   HWND hStaticRightArrow,
								   UINT nId)
{
	...
	::GetWindowRect(hStaticLeftArrow,&m_rectLeftArrow2);
	pParentWnd->ScreenToClient(&m_rectLeftArrow2);

	::GetWindowRect(hStaticRightArrow,&m_rectRightArrow2);
	pParentWnd->ScreenToClient(&m_rectRightArrow2);

	// hide placeholder window
	::ShowWindow(hStatic, SW_HIDE);

	return CreateFromRect(dwStyle, pParentWnd, rect,m_rectLeftArrow2, m_rectRightArrow2,nId);
}
...
BOOL CXScrollBar::CreateFromRect(DWORD dwStyle,
								 CWnd* pParentWnd,
								 CRect& rect,
								 CRect& rectLeftArrow2,
								 CRect& rectRightArrow2,
								 UINT nId)
{
	...
	CButton *pBtnL = new CButton;
	CButton *pBtnR = new CButton;

	pBtnL->Create(_T(""), WS_CHILD|WS_VISIBLE|BS_PUSHBUTTON|BS_BITMAP,rectLeftArrow2, pParentWnd, 1111);
	pBtnR->Create(_T(""), WS_CHILD|WS_VISIBLE|BS_PUSHBUTTON|BS_BITMAP,rectRightArrow2, pParentWnd, 2222);
	

	HBITMAP hBitmapL;
	hBitmapL = ::LoadBitmap(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDB_HORIZONTAL_SCROLLBAR_LEFTARROW2));
	HBITMAP hBitmapR;
	hBitmapR = ::LoadBitmap(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDB_HORIZONTAL_SCROLLBAR_RIGHTARROW2));

	pBtnL->SetBitmap(hBitmapL);
	pBtnR->SetBitmap(hBitmapR);
	...
	if (bResult)
	{
		...
		if (m_bmpThumb.GetSafeHandle())
		{
			...
			m_rectDownArrow = CRect(m_rectClient.left, m_rectClient.bottom - m_nBitmapHeight, 
				m_rectClient.right, m_rectClient.bottom);
		}
		else
		{
		...
		VERIFY(m_bmpUpLeftArrow2.LoadBitmap(m_bHorizontal ?
											IDB_HORIZONTAL_SCROLLBAR_LEFTARROW2 :
											IDB_VERTICAL_SCROLLBAR_UPARROW));

		VERIFY(m_bmpDownRightArrow2.LoadBitmap(m_bHorizontal ?
												IDB_HORIZONTAL_SCROLLBAR_RIGHTARROW2 :
												IDB_VERTICAL_SCROLLBAR_DOWNARROW));
	}
	else
	{
...
//=============================================================================
{
	...
	memDC.StretchBlt(rectDown.left, rectDown.top, 
		rectDown.Width()-1, rectDown.Height(), 
		&bitmapDC, 0, 0, m_nBitmapWidth, m_nBitmapHeight, SRCCOPY);
		...
		for (int x = 0; x < m_nBitmapWidth; x++)
		{
			for (int y = 0; y < m_nBitmapHeight; y++)
			{
				...
				if (rgb	== THUMB_UP_TRANSPARENT_MASK_COLOR)
					...
					bitmapDC.SetPixel(x, y, memDC.GetPixel(x, nChannelStart+nChannelHeight-1));
			}
		}
