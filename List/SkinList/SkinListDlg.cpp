// SkinListDlg.cpp : implementation file
//

#include "stdafx.h"
#include "SkinList.h"
#include "SkinListDlg.h"
...
CSkinListDlg::CSkinListDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CSkinListDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CSkinListDlg)
	//}}AFX_DATA_INIT
	// Note that LoadIcon does not require a subsequent DestroyIcon in Win32
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CSkinListDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CSkinListDlg)
	DDX_Control(pDX, IDC_SKIN_LIST, m_SkinList);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CSkinListDlg, CDialog)
	//{{AFX_MSG_MAP(CSkinListDlg)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
...
void CSkinListDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
HCURSOR CSkinListDlg::OnQueryDragIcon()
{
	return (HCURSOR) m_hIcon;
}


void CSkinListDlg::OnEndtrackSkinList(NMHDR* pNMHDR, LRESULT* pResult) 
{
	HD_NOTIFY *phdn = (HD_NOTIFY *) pNMHDR;
	m_SkinList.m_SkinHorizontalScrollbar.UpdateThumbPosition();
	*pResult = 0;
}
