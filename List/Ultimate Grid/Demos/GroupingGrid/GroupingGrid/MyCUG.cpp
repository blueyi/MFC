...
void MyCug::OnSetup()
{
	...
	QuickSetText(4,-1,_T("Winner"));
	JoinCells(0,-1,1,-1);
	QuickSetText(0,-1,_T("Team A"));
	JoinCells(2,-1,3,-1);
	QuickSetText(2,-1,_T("Team B"));

	for(int i=0; i<28; i++)
		QuickSetNumber(-1,i,i);

	QuickSetCellType(0,0,m_nExpandIndex);
	QuickSetText(0,0,_T("Pool A"));	
	JoinCells(0,0,4,0);		

	QuickSetText(0,1,arrTeams[0]);	
	QuickSetText(2,1,arrTeams[1]);	
	QuickSetText(0,2,arrTeams[2]);	
	QuickSetText(2,2,arrTeams[1]);	
	QuickSetText(0,3,arrTeams[2]);	
	QuickSetText(2,3,arrTeams[0]);	

	for(int i=1; i<=3; i++)
	{
		QuickSetNumber(1,i,0);
		GetCell(1,i,&cell);
		cell.SetReadOnly(FALSE);
		SetCell(1,i,&cell);
		QuickSetNumber(3,i,0);
		GetCell(3,i,&cell);
		cell.SetReadOnly(FALSE);
		SetCell(3,i,&cell);
	}

	QuickSetCellType(0,5,m_nExpandIndex);
	QuickSetText(0,5,_T("Pool B"));
	JoinCells(0,5,4,5);

	QuickSetText(0,6,arrTeams[3]);	
	QuickSetText(2,6,arrTeams[4]);	
	QuickSetText(0,7,arrTeams[5]);	
	QuickSetText(2,7,arrTeams[4]);	
	QuickSetText(0,8,arrTeams[5]);	
	QuickSetText(2,8,arrTeams[3]);
	...
	QuickSetCellType(0,10,m_nExpandIndex);
	QuickSetText(0,10,_T("Pool C"));
	JoinCells(0,10,4,10);
	m_expand.SetNumberOfRows(0,10,4);

	QuickSetText(0,11,arrTeams[6]);	
	QuickSetText(2,11,arrTeams[7]);	
	QuickSetText(0,12,arrTeams[8]);	
	QuickSetText(2,12,arrTeams[7]);	
	QuickSetText(0,13,arrTeams[8]);	
	QuickSetText(2,13,arrTeams[6]);
	...
	QuickSetCellType(0,15,m_nExpandIndex);
	QuickSetText(0,15,_T("Pool Results"));
	JoinCells(0,15,4,15);	

	for(int i=1; i<4; i++)
	{
		QuickSetText(4,i,_T("Draw"));
		QuickSetText(4,i+5,_T("Draw"));
		QuickSetText(4,i+10,_T("Draw"));
	}

	QuickSetText(0,16,_T("Team"));
	QuickSetText(1,16,_T("Win"));
	QuickSetText(2,16,_T("Loss"));
	QuickSetText(3,16,_T("Draw"));
	QuickSetText(4,16,_T("Points"));

	for(int i=18, j=0; i<27; i++, j++)
	{
		QuickSetText(0,i,arrTeams[j]);
		QuickSetNumber(1,i,0);
		QuickSetNumber(2,i,0);
		QuickSetNumber(3,i,2);
		QuickSetNumber(4,i,100);
	}
}
...
int MyCug::OnEditFinish(int col, long row,CWnd *edit,LPCTSTR string,BOOL cancelFlag)
{
	...
	int a = _ttoi(QuickGetText(1,row));
	int b = _ttoi(QuickGetText(3,row));
	...
	CString TeamA = QuickGetText(0,row);
	CString TeamB = QuickGetText(2,row);

	CString winner = _T("Draw");
	CString loser;
	if(a > b)
	{
		winner = TeamA;
	...
	CString origwinner = QuickGetText(4,row);

	if(winner.Compare(origwinner) != 0)
	{
		QuickSetText(4, row, winner);
		...
		for(int i=0; i<9; i++)
		{
			if(_tcscmp(TeamA, arrTeams[i]) == 0)
			{
				teamArow += i; 
				continue;
			}
			if(_tcscmp(TeamB, arrTeams[i]) == 0)
			{
				teamBrow += i; 			
			}			
		}	
		...
		if(a == b) // Draw
		{
			int d = 0;
			//Update Team A
			d = _ttoi(QuickGetText(3,teamArow));
			QuickSetNumber(3,teamArow, d+1);			

			//Update Team B
			d = _ttoi(QuickGetText(3,teamBrow));
			QuickSetNumber(3,teamBrow, d+1);

			int origwinrow = 0, origlossrow = 0;
			if(origwinner.Compare(TeamA) == 0)
			{
				origwinrow = teamArow;
				origlossrow = teamBrow;
			}
			else // Team B is original winner
			{
				origwinrow = teamBrow;
				origlossrow = teamArow;
			}

			int w, l, pts;
			w = _ttoi(QuickGetText(1,origwinrow));
			QuickSetNumber(1,origwinrow, w-1);
			pts = _ttoi(QuickGetText(4,origwinrow));
			QuickSetNumber(4,origwinrow, pts-50);

			l = _ttoi(QuickGetText(2,origlossrow));
			QuickSetNumber(2,origlossrow, l-1);
			pts = _ttoi(QuickGetText(4,origlossrow));
			QuickSetNumber(4,origlossrow, pts+50);
		}
		else // One team won
		{
			int winrow, lossrow;
			...
			int w, l;
			w = _ttoi(QuickGetText(1,winrow));
			QuickSetNumber(1,winrow, w+1);
			l = _ttoi(QuickGetText(2,lossrow));
			QuickSetNumber(2,lossrow, l+1);


			int pts;
			if(origwinner.Compare(_T("Draw")) == 0) // Previously drawn game
			{
				int d;
				d = _ttoi(QuickGetText(3,winrow));
				QuickSetNumber(3,winrow, d-1);
				pts = _ttoi(QuickGetText(4,winrow));
				QuickSetNumber(4,winrow, pts+50);

				d = _ttoi(QuickGetText(3,lossrow));
				QuickSetNumber(3,lossrow, d-1);
				pts = _ttoi(QuickGetText(4,lossrow));
				QuickSetNumber(4,lossrow, pts-50);
			}
			else
			{
				w = _ttoi(QuickGetText(1,lossrow));
				QuickSetNumber(1,lossrow, w-1);
				pts = _ttoi(QuickGetText(4,lossrow));
				QuickSetNumber(4,lossrow, pts-100);

				l = _ttoi(QuickGetText(2,winrow));
				QuickSetNumber(2,winrow, l-1);
				pts = _ttoi(QuickGetText(4,winrow));
				QuickSetNumber(4,winrow, pts+100);
			}
		}
