...
//#ifdef _DEBUG
				...
				AfxMessageBox("HTTP Query Failed");
				TRACE ("HTTP Query Failed\n");
			}
//#endif
		}
		catch(CException* e)
...
void CUG_HTML_DataSource::InterpretTags(CString text,CUGCell* cell)
{
	...
	// take out extra tags
	while ((i=text.Find("<")) !=-1)
		text = text.Left(i) + text.Mid(text.Find(">")+1);
	// take out extra line feed, carriage returns, tabs etc.
	while ((i=text.FindOneOf("\f\r\n\t")) !=-1)
