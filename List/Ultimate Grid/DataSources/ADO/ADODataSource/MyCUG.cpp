...
void MyCug::OnSetup()
{
	...
	// bind the datasource to data, provide it with open and
	// connection information
	// m_dsADO.OpenSQL( "SELECT * FROM customers;", "Data Source=test" );
	m_dsADO.OpenSQL( "SELECT * FROM table1;", "Data Source=Test1" );

	// add the datasource
	AddDataSource( &m_dsADO );
	// set added datasource to be default
	SetDefDataSource( m_dsADO.GetID());
	// complete the datasource binding operation, let grid adjust
	// itself to reflect information found in the datasource
	SetGridUsingDataSource( m_dsADO.GetID());
