...
HITEM CPropertyGrid::AddItem(HSECTION hs, EItemType type, string name, void* pValue, bool editable, bool undefined, HITEM after)
{
  ...
  else if (type == IT_BOOLEAN || type == IT_CHECK ) item.m_bValue = *(bool*)pValue;
...
bool CPropertyGrid::GetItemValue(HITEM item, bool& bValue) const
{
  ...
  // check
  if (pItem->m_type == IT_BOOLEAN || pItem->m_type == IT_CHECK )
  {
...
void CPropertyGrid::DrawItem(CDC& dc, int w, int x, int y, vector<CItem>::iterator& it)
{
  ...
  if (m_focused_item == it->m_id && it->m_editable && GetEditMode(*it) != EM_CUSTOM)
  {
    ...
    case EM_INPLACE:
      ...
      break;
		...
		case EM_IMMEDIATE:
			m_rect_button = AdjustCheckBoxRect(&CRect(value_left, y, value_right, y+m_line_height));
			break;
  ...
  else
  {
    ...
    case IT_FOLDER:
      {
        TCHAR szBuffer[1024];
        strncpy(szBuffer, strValue.c_str(), 1024);
    ...
    case IT_FONT:
      {
		...
		case IT_CHECK:
			{
				// draw a sample rectangle
				CRect rc = AdjustCheckBoxRect(&(it->m_rcValue));
				BITMAPINFO bmi;
				bmi.bmiHeader.biBitCount = 32;
				bmi.bmiHeader.biClrImportant = 0;
				bmi.bmiHeader.biClrUsed = 0;
				bmi.bmiHeader.biCompression = BI_RGB;
				bmi.bmiHeader.biHeight = CheckBoxHeight;
				bmi.bmiHeader.biPlanes = 1;
				bmi.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
				bmi.bmiHeader.biSizeImage = 0; //0 is ok for BI_RGB (uncompressed)
				bmi.bmiHeader.biWidth = CheckBoxHeight;
				bmi.bmiHeader.biXPelsPerMeter = 1;
				bmi.bmiHeader.biYPelsPerMeter = 1;
				
				void* pBuffer;
				if ( it->m_editable )
					pBuffer = it->m_bValue? &Item_CheckOn : &Item_CheckOff;
				else
					pBuffer = it->m_bValue? &Item_CheckOn_Disabled : &Item_CheckOff_Disabled;
				StretchDIBits(dc.m_hDC, rc.left, rc.top, rc.Width(), rc.Height(), 0, CheckBoxHeight+1, CheckBoxHeight, -CheckBoxHeight, pBuffer, &bmi, DIB_RGB_COLORS, SRCCOPY);
				break;
			}
    }
...
void CPropertyGrid::OnLButtonDown(UINT nFlags, CPoint point)
{
  ...
  {
    ASSERT(m_focused_item != -1);
		CItem* pItem = FindItem(m_focused_item);
		if ( pItem && pItem->m_type == IT_CHECK && pItem->m_editable)
		{
          ...
          if (it2->m_editable || m_focus_disabled)
          {
            ...
            if (it2->m_rcValue.PtInRect(point))
              ...
              m_value_clicked = (GetEditMode(*it2) == EM_INPLACE || GetEditMode(*it2) == EM_DROPDOWN);
							if ( it2->m_type == IT_CHECK && it2->m_editable)
							{
								CRect rc = AdjustCheckBoxRect(&(it2->m_rcValue));
								if ( rc.PtInRect(point))
								{
									it2->m_bValue = !it2->m_bValue;
									GetOwner()->SendMessage(WM_PG_ITEMCHANGED, it2->m_id); 
								}
							}
						}
            Invalidate();
      ...
      if (pItem)
      {
        ...
        else if (GetEditMode(*pItem) == EM_CUSTOM)
        {
          pItem->m_pCustom->OnLButtonUp(pItem->m_rcValue, point);
				...
				else if (GetEditMode(*pItem) == EM_IMMEDIATE)
				{
				}
        else
        {
