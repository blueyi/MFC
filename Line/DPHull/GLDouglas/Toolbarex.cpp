...
CWnd* CToolBarEx::InsertControl(CWnd* pCtrl, CRect & rect, UINT nID)
{
	...
	if (nIndex>-1)
	{
		...
		rect.top+=__max((rect.Height()-rt.Height())/2,0);	 //move to middle
...
CSize CToolBarEx::GetButtonSize(TBBUTTON* pData, int iButton, DWORD dwMode)
{
	...
	if (dwStyle & TBSTYLE_SEP)
		sz.cy = __min(sz.cy ,HIWORD(GetToolBarCtrl().GetButtonSize()));
...
void CToolBarEx::SizeToolBar(TBBUTTON* pData, int nCount, int nLength, BOOL bVert, DWORD dwMode)
{
	...
	if (!bVert)
	{
		int nMin, n__max, nTarget, nCurrent, nMid;

		// Wrap ToolBar as specified
		n__max = nLength;
		nTarget = WrapToolBar(pData, nCount, n__max,dwMode);
		...
		if (nCurrent != nTarget)
		{
			while (nMin < n__max)
			{
				...
				else
				{
					if (nMin == nMid)
					{
						WrapToolBar(pData, nCount, n__max,dwMode);
						break;
					}
					nMin = nMid;
				}
	...
	{
		CSize size__max, sizeMin, sizeMid;
		...
		size__max = CalcSize(pData, nCount,dwMode);

		while (sizeMin.cx < size__max.cx)
		{
			sizeMid.cx = (sizeMin.cx + size__max.cx) / 2;
			...
			if (nLength < sizeMid.cy)
			{
				if (sizeMin == sizeMid)
				{
					WrapToolBar(pData, nCount, size__max.cx,dwMode);
					return;
				}
				sizeMin = sizeMid;
			}
					...
					while(pWnd)
					{
					  ...
					  int id =pWnd->GetDlgCtrlID();
						...
						if (nIndex>-1)
						{
							...
							CRect rt;
							pWnd->GetWindowRect(&rt);
							rect.top+=__max((rect.Height()-rt.Height())/2,0);
		...
		if (pBar != NULL)
		{
			...
			if (pBar->IsVisible())
			{
				...
				if (bHorz)
				{
					...
					if (rect.right > size__max.cx && !m_bFloating)
					{
						int x = rect.Width() - cxBorder;
						x = __max(size__max.cx - x, pt.x);
					...
					else if ((rect.left >= (size__max.cx - cxBorder)) &&
				...
				else
				{
					...
					if (rect.bottom > size__max.cy && !m_bFloating)
					{
						int y = rect.Height() - cyBorder;
						y = __max(size__max.cy - y, pt.y);
					...
					// If ControlBar is completely invisible, then wrap it
					else if ((rect.top >= (size__max.cy - cyBorder)) &&
						(nPos > 0) && (m_arrBars[nPos - 1] != NULL))
					{
						m_arrBars.InsertAt(nPos, (CObject*)NULL);
						pBar = NULL; pVoid = NULL;
