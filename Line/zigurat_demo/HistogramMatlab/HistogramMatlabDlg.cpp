...
void CHistogramMatlabDlg::ShowHistogram(ERandType eType, UINT nSteps)
{
	...
	std::vector<double> vSamples(n);

	switch (eType)
	{
	case ENormal:
		for (i = 0; i < n; i++)
			vSamples[i]=(double)CRandomGenerator::RNOR();
		break;
	case EExponential:
		for (i = 0; i < n; i++)
			vSamples[i]=(double)CRandomGenerator::REXP();
		break;
	}
	
	matlab.PutMatrix(_T("sample"), vSamples, vSamples.size(), 1);
	matlab.Execute(_T("hist(sample,100)"));

	UINT nRows = 0, nCols = 0;
	matlab.GetMatrix(_T("sample"), vSamples, nRows, nCols);
