...
//
//bool operator == (const matrixT& m1, const matrixT& m2) _NO_THROW
//bool operator != (const matrixT& m1, const matrixT& m2) _NO_THROW
//matrixT operator + (const matrixT& m1, const matrixT& m2) _THROW_MATRIX_ERROR
//matrixT operator - (const matrixT& m1, const matrixT& m2) _THROW_MATRIX_ERROR
//matrixT operator * (const matrixT& m, const T& no) _NO_THROW
//matrixT operator * (const T& no, const matrixT& m) _NO_THROW
//matrixT operator * (const matrixT& m1, const matrixT& m2) _THROW_MATRIX_ERROR
//matrixT times(const matrixT& m1,const matrixT& m2)_THROW_MATRIX_ERROR
//matrixT divide(const matrixT& m1,const matrixT& m2)_THROW_MATRIX_ERROR
//matrixT operator / (const matrixT& m, const T& no) _NO_THROW
//matrixT operator / (const T& no, const matrixT& m) _THROW_MATRIX_ERROR
//matrixT operator / (const matrixT& m1, const matrixT& m2) _THROW_MATRIX_ERROR
//matrixT operator ^ (const matrixT& m, const size_t& pow) _THROW_MATRIX_ERROR
//matrix<bool> operator || (const matrix<bool>& m1, const matrix<bool>& m2) _THROW_MATRIX_ERROR
//matrix<int> operator | (const matrix<int>& m1, const matrix<int>& m2) _THROW_MATRIX_ERROR
//matrix<bool> operator && (const matrix<bool>& m1, const matrix<bool>& m2) _THROW_MATRIX_ERROR
//matrix<int> operator & (const matrix<int>& m1, const matrix<int>& m2) _THROW_MATRIX_ERROR
...
//   matrix (const matrix<int>& m);
   ...
   matrixT operator() (const matrix<int>& m) _THROW_MATRIX_ERROR;
   matrixT operator() (const matrix<bool>&m) _THROW_MATRIX_ERROR;
   ...
   matrixT& operator += (const matrixT& m) _THROW_MATRIX_ERROR;
   matrixT& operator += (T c) _NO_THROW;
   matrixT& operator -= (const matrixT& m) _THROW_MATRIX_ERROR;
   matrixT& operator -= (T c) _NO_THROW;
   matrixT& operator *= (const matrixT& m) _THROW_MATRIX_ERROR;
   ...
   //compare operator
   matrix<bool> operator >  (const T&c) _NO_THROW;
   matrix<bool> operator >= (const T&c) _NO_THROW;
   matrix<bool> operator <  (const T&c) _NO_THROW;
   matrix<bool> operator <= (const T&c) _NO_THROW;
   matrix<bool> operator == (const T&c) _NO_THROW;
...
matrixT::operator () (const matrix<int>& m) _THROW_MATRIX_ERROR
{
	 matrixT temp(m.RowNo(),m.ColNo());
		for(int i=0;i<m.RowNo();i++)
			for(int j=0;j<m.ColNo();j++)
			{//row first
				int _row=m(i,j)/_m->Row;
				int _col=m(i,j)%_m->Row;
				temp(i,j)=(*this)(_row,_col);
			}
		return temp;
}
...
MAT_TEMPLATE inline matrix<bool> 
matrixT::operator >(const T&c) _NO_THROW
{
	matrix<bool>temp(_m->Row,_m->Col);
	for(size_t i=0;i<_m->Row;i++)
		for(size_t j=0;j<_m->Col;j++)
			temp(i,j)=( (*this)(i,j) > c);
		return temp;
...
MAT_TEMPLATE inline matrixT
operator + (const matrixT& m1, T c) _THROW_MATRIX_ERROR
{
	matrixT temp = m1;
	temp += c;
	return temp;
...
MAT_TEMPLATE inline matrixT
operator + (T c,const matrixT& m1 ) _THROW_MATRIX_ERROR
{
	matrixT temp = m1;
	temp += c;
	return temp;
...
// binary array multiplication operator
MAT_TEMPLATE inline matrixT
times(const matrixT& m1,const matrixT& m2)_THROW_MATRIX_ERROR
{
	if ( m1.RowNo()!=m2.RowNo() || m1.ColNo()!=m2.ColNo() )
		REPORT_ERROR("times() :Inconsistent matrix sizes in array times!");
	matrixT temp=m1;
...
// binary array multiplication operator
MAT_TEMPLATE inline matrixT
divide(const matrixT& m1,const matrixT& m2)_THROW_MATRIX_ERROR
{
	if ( m1.RowNo()!=m2.RowNo() || m1.ColNo()!=m2.ColNo() )
		REPORT_ERROR("divide() :Inconsistent matrix sizes in array divide!");
	matrixT temp=m1;
	for(int i=0;i<m1.RowNo();i++)
		for(int j=0;j<m1.ColNo();j++)
			temp(i,j)/=m2(i,j);
...
inline matrix<bool>
operator || (const matrix<bool>& m1, const matrix<bool>& m2) _THROW_MATRIX_ERROR
{
	if (m1.RowNo()!=m2.RowNo() || m1.ColNo()!=m2.ColNo())
		REPORT_ERROR("operater || :Inconsistent matrix sizes in operator || !");
		
	matrix<bool> temp = m1;
	for(int i=0;i<m1.RowNo();i++)
		for(int j=0;j<m1.ColNo();j++)
			temp(i,j) = (temp(i,j) || m2(i,j)) ;
		return temp;
}
inline matrix<int>
operator | (const matrix<int>& m1, const matrix<int>& m2) _THROW_MATRIX_ERROR
{
	if (m1.RowNo()!=m2.RowNo() || m1.ColNo()!=m2.ColNo())
		REPORT_ERROR("operater | :Inconsistent matrix sizes in operator | !");
