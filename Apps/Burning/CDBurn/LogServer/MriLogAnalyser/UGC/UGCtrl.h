...
public:
	...
	void	 SetReportColWidth(int nCol,int nWidth);
	inline void FormatOneReportLine(int nRow,vector<string> &vecFormatStr,string &strLine);
	inline int  MakeReportFormatString(vector<string> &vecFormatStr);
	bool SaveGridReport(LPCSTR lpcszFilename);
	void SaveGridToReportFile(CStdioFile &file);

private:
	vector<int>		m_vecReportColWidth;
};

/////////////////////////////////////////////////////////////////////////////
inline int  CUGCtrl::MakeReportFormatString(vector<string> &vecFormatStr)
{
	int nLen=0;
	char szBuf[20];
	int nSize=m_vecReportColWidth.size();
	vecFormatStr.resize(nSize);
	for(int i=0;i<nSize;i++)
	{
		nLen+=m_vecReportColWidth[i];
		strcpy(szBuf,"%s");
		vecFormatStr[i]=szBuf;
	}
	return nLen;
}
inline void CUGCtrl::FormatOneReportLine(int nRow,vector<string> &vecFormatStr,string &strLine)
{
	int nPos=0;
	char *pBuf=(char *)(strLine.c_str());
	CUGCell cell;
	CString strTemp;
	for(int i=0;i<m_vecReportColWidth.size();i++)
	{
		OnGetCell(i,nRow,&cell);
		cell.GetText(&strTemp);
		int nStrLen=strTemp.GetLength();
		sprintf(pBuf+nPos,vecFormatStr[i].c_str(),strTemp.GetBuffer(strTemp.GetLength()));
		int nSize=m_vecReportColWidth[i]-nStrLen;
		if(nSize>0)
			memset(pBuf+nPos+nStrLen,' ',nSize);
		nPos+=m_vecReportColWidth[i];			
	}
	pBuf[nPos]='\n';
	pBuf[nPos+1]='\0';
}
