...
void CUGCtrl::OnSysColorChange() 
{
	...
	m_GI->m_gridDefaults->SetBackColor( GetSysColor( COLOR_BTNFACE )/*gnGridBkColor*/ );
...
void CUGCtrl::SetReportColWidth(int nCol,int nWidth)
{
	int nSize=m_vecReportColWidth.size();
	if(nCol>=nSize)
		m_vecReportColWidth.resize(nCol+1);
	for(int i=nSize;i<nCol-1;i++)
	{
		m_vecReportColWidth[i]=10;
	}
	m_vecReportColWidth[nCol]=nWidth;
}


bool CUGCtrl::SaveGridReport(LPCSTR lpcszFilename)
{
	CStdioFile file;
	if(file.Open(lpcszFilename,CFile::modeCreate|CFile::modeWrite,NULL)!=TRUE)
		return false;
	SaveGridToReportFile(file);
	file.Close();
	return true;
} 
void CUGCtrl::SaveGridToReportFile(CStdioFile &file)
{
	string strLine;
	vector<string> vecFormatStr;
	int nLineLen=MakeReportFormatString(vecFormatStr);
	strLine.reserve(nLineLen*1.2);
	int nRowNumber=GetNumberRows();

	FormatOneReportLine(-1,vecFormatStr,strLine);
	file.WriteString(strLine.c_str());
	memset((char *)(strLine.c_str()),'-',nLineLen);
	file.WriteString(strLine.c_str());
	for(int nRowCount=0;nRowCount<nRowNumber;nRowCount++)
	{
		FormatOneReportLine(nRowCount,vecFormatStr,strLine);
		file.WriteString(strLine.c_str());
	}
}