...
BOOL CMarkupDlg::OnInitDialog()
{
	...
	if ( bTestingFileLoaded )
	{
		...
		xmlSettings.AddChildElem( _T("Test") );
		xmlSettings.SetChildData( (LPCTSTR)csTestLog, 1 );
		xmlSettings.SetChildAttrib( _T("t"), (LPCTSTR)CTime::GetCurrentTime().Format("%d-%b-%Y %H:%M:%S" ) );
