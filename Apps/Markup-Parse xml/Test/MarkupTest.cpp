...
#define MARKUP_RELEASE MCD_T("11.2")
...
int CMarkupTest::RunTest()
{
	...
	m_strResult = MCD_T("CMarkup ");
	m_strResult += MARKUP_RELEASE;
	m_strResult += MCD_T(" ");;
#if defined(MARKUP_MSXML)
	m_strResult += MCD_T("MSXML");
	...
	m_strResult += MCD_T(" STL");
#else // not STL
	m_strResult += MCD_T(" MFC");
#endif // not STL
#if defined(MARKUP_ICONV)
	m_strResult += MCD_T(" ICONV");
#endif // ICONV
#if defined(MARKUP_WINCONV)
	m_strResult += MCD_T(" WINCONV");
#endif // WINCONV
#if defined(MARKUP_WCHAR)
	m_strResult += MCD_T(" WCHAR");
#endif // WCHAR
#if defined(MARKUP_MBCS)
	m_strResult += MCD_T(" MBCS");
#endif // MBCS
#if defined(_DEBUG)
	m_strResult += MCD_T(" Debug");
#endif // DEBUG
	if ( sizeof(MCD_INTFILEOFFSET) > 4 )
		m_strResult += MCD_T(" HUGE"); // file mode supports files over 2GB
	m_strResult += MCD_T("\r\n");
	...
	nCreateSpeed = nPerfCreateTime? nPerfCreateLength/nPerfCreateTime: 0;
	nParseSpeed = nPerfParseTime? nPerfParseLength/nPerfParseTime: 0;
	...
	{
		MCD_CHAR szAttribTime[100];
		MCD_SPRINTF( MCD_SSZ(szAttribTime), MCD_T("Attribute Performance Results: create %d search %d\r\n"),
			nAttribCreateTime, nAttribNavTime );
		m_strResult += szAttribTime;
	}
	...
	MCD_SPRINTF( MCD_SSZ(szResults), MCD_T("%d checks, create %d%s/ms, parse %d%s/ms"),
		m_nTotalChecks, nCreateSpeed, pszCharType, nParseSpeed, pszCharType );
	m_strResult += szResults;
	if ( nWriterSpeed )
	{
		MCD_SPRINTF( MCD_SSZ(szResults), MCD_T(", writer %db/ms, reader %db/ms"), nWriterSpeed, nReaderSpeed );
		m_strResult += szResults;
	}
	m_strResult += MCD_T("\r\n");
	if ( ! m_bLSFileLoaded )
...
void CMarkupTest::LoadParse( MCD_CSTR_FILENAME pszPath )
{
	...
	int nTime = GetMilliSpan( nTimeStart );

	// Display results
	MCD_CHAR szResult[100];
	if ( bResult )
	{
		MCD_SPRINTF( MCD_SSZ(szResult), MCD_T("Loaded and parsed in %d milliseconds"), nTime );
	}
	else
	{
		MCD_SPRINTF( MCD_SSZ(szResult), MCD_T("Load or parse error (after %d milliseconds)"), nTime );
	}
