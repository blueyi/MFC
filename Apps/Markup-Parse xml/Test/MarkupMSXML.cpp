...
#else // UTF-8
	int nUTF16Len = MultiByteToWideChar(CP_UTF8,0,pszText,-1,NULL,0);
	wchar_t* pwszUTF16 = new wchar_t[nUTF16Len];
	MultiByteToWideChar(CP_UTF8,0,pszText,-1,pwszUTF16,nUTF16Len);
	_bstr_t bstrText( pwszUTF16 );
	delete [] pwszUTF16;
#endif
	return bstrText;
}
...
CString CMarkupMSXML::FromBSTR( const _bstr_t& bstrText )
{
	...
	if ( bstrText.length() )
	{
		int nUTF8Len = WideCharToMultiByte(CP_UTF8,0,(LPCWSTR)bstrText,-1,NULL,0,NULL,NULL);
		WideCharToMultiByte(CP_UTF8,0,(LPCWSTR)bstrText,-1,csText.GetBuffer(nUTF8Len),nUTF8Len,NULL,NULL);
		csText.ReleaseBuffer(nUTF8Len-1);
	}
#endif
	return csText;
}
...
_variant_t CMarkupMSXML::ToVARIANT( LPCTSTR pszText )
{
	...
	MultiByteToWideChar(CP_UTF8,0,pszText,-1,pwszUTF16,nUTF16Len);
	varText.vt = VT_BSTR;
	varText.bstrVal = SysAllocString(pwszUTF16);
	delete [] pwszUTF16;
#endif
	return varText;
}
	...
	while ( aVer[nTry].nVer )
	{
		if ( m_nVer == 0 || m_nVer == aVer[nTry].nVer )
		{
			...
			if ( SUCCEEDED(hr) )
			{
				...
				// pDOMDoc->PutpreserveWhiteSpace( VARIANT_TRUE );
				if ( m_nVer == 6 )
...
bool CMarkupMSXML::x_GetReason()
{
	m_strResult.Empty();
	MSXMLNS::IXMLDOMParseErrorPtr pDOMParseError;
	m_pDOMDoc->get_parseError( &pDOMParseError );
	BSTR bstrReason;
	HRESULT hr = pDOMParseError->get_reason( &bstrReason );
	if ( SUCCEEDED(hr) )
	{
		CString strError = FromBSTR(bstrReason);
		strError.TrimRight( _T("\r\n") );
		CMarkupMSXML msxmlResult;
		msxmlResult.AddElem( _T("error") );
		msxmlResult.SetAttrib( _T("msg"), strError );
		long nLine = 0, nLinePos = 0;
		HRESULT hrLine = pDOMParseError->get_line( (long*)&nLine );
		HRESULT hrCol = pDOMParseError->get_linepos( (long*)&nLinePos );
		if ( SUCCEEDED(hrLine) && SUCCEEDED(hrCol) )
		{
			msxmlResult.SetAttrib( _T("line"), nLine );
			msxmlResult.SetAttrib( _T("linepos"), nLinePos );
		}
		m_strResult = msxmlResult.GetDoc();
	}
	return false;
}

bool CMarkupMSXML::x_AddResult( CString strID )
{
	CMarkupMSXML msxmlResult;
	msxmlResult.AddElem( strID );
	m_strResult = msxmlResult.GetDoc();
	return false;
}

CString CMarkupMSXML::GetError() const
{
	CString strError;
	if ( m_strResult.GetLength() )
	{
		CMarkupMSXML msxmlResult( m_strResult );
		msxmlResult.FindElem();
		CString strTagName = msxmlResult.GetTagName();
		if ( strTagName == _T("error") )
		{
			strError = msxmlResult.GetAttrib( _T("msg") );
			CString strLine = msxmlResult.GetAttrib( _T("line") );
			CString strLinePos = msxmlResult.GetAttrib( _T("linepos") );
			if ( strLine.GetLength() )
				strError += _T(" at line ") + strLine +  _T(" pos ") + strLinePos;
		}
		else if ( strTagName == _T("target_mismatch") )
			strError = _T("Specified Target does not match current node");
		else if ( strTagName == _T("doctype_readonly") )
			strError = _T("Document Type is Read-Only");
	}
	return strError;
}
...
#if ! defined(MARKUP_MSXML1)
CString CMarkupMSXML::Transform( CMarkupMSXML& xsl )
{
	CString csResult;
	m_strResult.Empty();
	BSTR bstrOut;
	HRESULT hrXSL = S_OK;
	m_pDOMDoc->raw_transformNode( xsl.m_pDOMDoc, &bstrOut );
	if ( SUCCEEDED(hrXSL) )
		csResult = FromBSTR( bstrOut );
	else
		x_GetReason();
	return csResult;
}
...
bool CMarkupMSXML::x_AddNode( int nNodeType, LPCTSTR szText, bool bInsert )
{
	...
	case MNT_DOCUMENT_TYPE:
		return x_AddResult( _T("doctype_readonly") );
	}
		...
		case MNT_PROCESSING_INSTRUCTION:
			{
				...
				{
					return x_AddResult( _T("target_mismatch") );
				}
