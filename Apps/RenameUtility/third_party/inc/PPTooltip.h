...
public:
	...
	void	AddTool(CWnd *pWnd, DWORD dwIdString, DWORD dwIdIcon, CSize &szIcon = CSize(0, 0), LPCRECT lpRectBounds = NULL,
					DWORD dwIDTool = 0);
	void	AddTool(CWnd *pWnd, LPCTSTR lpszString, HICON hIcon, LPCRECT lpRectBounds = NULL, DWORD dwIDTool = 0);
	void	AddTool(CWnd *pWnd, LPCTSTR lpszString, DWORD dwIdIcon, CSize &szIcon = CSize(0, 0),
					LPCRECT lpRectBounds = NULL, DWORD dwIDTool = 0);
	...
	inline BYTE GetTransparency()
	{
		return(m_nTransparency);
	};

	//Shadow of the tooltip
	void	SetTooltipShadow(int nOffsetX, int nOffsetY, BYTE nDarkenPercent = 50, BOOL bGradient = TRUE, int nDepthX = 7,
							 int nDepthY = 7);
	void	SetImageShadow(int nOffsetX, int nOffsetY, BYTE nDarkenPercent = 50, BOOL bGradient = TRUE, int nDepthX = 7,
						   int nDepthY = 7);

#ifdef PPTOOLTIP_USE_MENU
	//Methods for the menu
	void	MenuToolPosition(DWORD nPos = PPTOOLTIP_MENU_LEFT | PPTOOLTIP_MENU_TOP);
	inline DWORD GetMenuToolPosition()
	{
		return(m_dwMenuToolPos);
	};
...
protected:
	...
	{
		return((GetStyle() & WS_VISIBLE) == WS_VISIBLE);
	}
	...
	DWORD			GetTooltipDirection(DWORD dwDirection, const LPPOINT lpPoint, LPPOINT lpAnchor, CRect &rcBody,
										CRect &rcFull, CRect &rcTipArea);
