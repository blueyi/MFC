...
void CPPToolTip::OnRedrawTooltip(HDC hDC, BYTE nTransparency /* = 0 */ )
{
	...
	:: BitBlt(hMemDC, 0, 0, rect.Width(), rect.Height(), hBkDC, 0, 0, SRCCOPY);
	...
	if(NULL == m_hUnderTooltipBk)
	{
		...
		:: BitBlt(hBkDC,
				  ...
				  hDC,
				  m_rcBoundsTooltip.left,
				  m_rcBoundsTooltip.top,
				  SRCCOPY);
	}
	else
	{
	...
	m_drawer.GetDrawManager()->AlphaBitBlt(hTempDC,
										   m_rcBoundsTooltip.left,
										   m_rcBoundsTooltip.top,
										   rect.Width(),
										   rect.Height(),
					...
					else
					{
						...
						else
						{
							...
							else if(!(m_tiDisplayed.nBehaviour & PPTOOLTIP_CLOSE_LEAVEWND))
							{
								...
								if((hWnd == m_hwndDisplayedTool) && !(m_tiDisplayed.nBehaviour & PPTOOLTIP_MULTIPLE_SHOW))
								{
...
void CPPToolTip::SetNewTooltip(HWND					   hWnd,
							   const PPTOOLTIP_INFO	   &ti,
							   BOOL					   bDisplayWithDelay /* = TRUE */,
							   TooltipType			   type /* = PPTOOLTIP_NORMAL */
						   )
{
	...
	if((PPTOOLTIP_STATE_SHOWING == m_nTooltipState) || (PPTOOLTIP_STATE_SHOWN == m_nTooltipState))
	{
		HideTooltip();
	}
...
void CPPToolTip::OnTimer(UINT nIDEvent)
{
	...
	switch(nIDEvent)
	{
		case TIMER_SHOW:
			...
			if((PPTOOLTIP_HELP == m_nNextTooltipType) || (PPTOOLTIP_MENU == m_nNextTooltipType))
			{
				pt = m_ptOriginal;
			}
			else
		...
		case TIMER_SHOWING:
			...
			{
				m_dwCurTransparency = PERCENT_MIN_TRANSPARENCY;
			}
...
void CPPToolTip::PrepareDisplayTooltip(LPPOINT lpPoint)
{
	...
	POINT	ptAnchor;
	m_dwCurDirection = GetTooltipDirection(m_tiNextTool.nDirection,
										   &pt,
										   &ptAnchor,
										   m_rcTooltip,
										   m_rcBoundsTooltip,
	...
	m_hrgnTooltip = GetTooltipRgn(m_dwCurDirection,
								  ptAnchor.x,
								  ptAnchor.y,
								  m_rcBoundsTooltip.Width(),
								  m_rcBoundsTooltip.Height());
	...
	switch(m_dwCurDirection)
	{
		...
		case PPTOOLTIP_LEFTEDGE_BOTTOM:
			m_drawer.GetDrawManager()->MultipleCopy(hMemDC,
													m_rcBoundsTooltip.left,
													m_rcBoundsTooltip.top,
													m_rcTooltip.left - m_rcBoundsTooltip.left,
													m_rcBoundsTooltip.Height(),
													hMemDC,
													m_rcTooltip.left + 1,
													m_rcBoundsTooltip.top,
		...
		case PPTOOLTIP_RIGHTEDGE_BOTTOM:
			m_drawer.GetDrawManager()->MultipleCopy(hMemDC,
													m_rcTooltip.right,
													m_rcBoundsTooltip.top,
													m_rcBoundsTooltip.right - m_rcTooltip.right,
													m_rcBoundsTooltip.Height(),
													hMemDC,
													m_rcTooltip.right - 1,
													m_rcBoundsTooltip.top,
		...
		case PPTOOLTIP_TOPEDGE_RIGHT:
			m_drawer.GetDrawManager()->MultipleCopy(hMemDC,
													...
													m_rcBoundsTooltip.top,
													m_rcBoundsTooltip.Width(),
													m_rcTooltip.top - m_rcBoundsTooltip.top,
													hMemDC,
													m_rcBoundsTooltip.left,
													m_rcTooltip.top + 1,
													m_rcBoundsTooltip.Width(),
...
DWORD CPPToolTip::GetTooltipDirection(DWORD			  dwDirection,
									  const LPPOINT	  lpPoint,
									  LPPOINT		  lpAnchor,
									  CRect			  &rcBody,
									  CRect			  &rcFull,
									  CRect			  &rcTipArea)
...
HRGN CPPToolTip::GetTooltipRgn(DWORD dwDirection, int x, int y, int nWidth, int nHeight)
{
	...
	hrgnBody = :: CreateRoundRectRgn(rcBody.left,
									 rcBody.top,
									 rcBody.right + 1,
									 rcBody.bottom + 1,
									 m_dwSizes[PPTTSZ_ROUNDED_CX],
									 m_dwSizes[PPTTSZ_ROUNDED_CY]);
...
CString CPPToolTip::GetDebugInfoTool(LPPOINT lpPoint)
{
	...
	//4. Window's ID
	str.Format(_T("<tr><td bgcolor=buttonface>Control ID</td><td bgcolor=white>%d</td></tr>"),
			   GetWindowLong(hWnd, GWL_ID));
	strOutput += str;

	//5. Window's styles
	str.Format(_T("<tr><td bgcolor=buttonface>Styles</td><td bgcolor=white>0x%08X</td></tr>"),
	...
	str.Format(_T("<tr><td bgcolor=buttonface>RECT</td><td bgcolor=white>(%d, %d)-(%d, %d)</td></tr>"),
			   rc.left,
			   rc.top,
			   rc.right,
			   rc.bottom);
...
////////////////////////////////////////////////////////////////////
void CPPToolTip::AddTool(CWnd	 *pWnd,
						 DWORD	 dwIdString,
						 HICON	 hIcon,
...
{
	CString str;
	str.Format(_T("<table><tr><td><icon handle=0x%X></td><td>%s</td></tr></table>"), hIcon, lpszString);
...
void CPPToolTip::ShowHelpTooltip(LPPOINT pt, LPCTSTR lpszString, HICON hIcon /* = NULL */ )
{
	...
	{
		ti.sTooltip.Format(_T("<table><tr><td><icon handle=0x%X></td><td>%s</td></tr></table>"), hIcon, lpszString);
	}	//if
...
////////////////////////////////////////////////////////////////////
void CPPToolTip::SetTooltipShadow(int	  nOffsetX,
								  int	  nOffsetY,
...
void CPPToolTip::SetImageShadow(int		nOffsetX,
								int		nOffsetY,
								BYTE	nDarkenPercent /* = 50 */,
								BOOL	bGradient /* = TRUE */,
								int		nDepthX /* = 7 */,
