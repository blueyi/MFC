...
void CPPHtmlDrawer::CallbackOnRepaint(int nIndexLink)
{
	...
	:: SendMessage(m_csCallbackRepaint.hWnd,
				   m_csCallbackRepaint.nMessage,
				   (LPARAM) nIndexLink,
				   m_csCallbackRepaint.lParam);
...
CSize CPPHtmlDrawer::DrawHtmlTable(CDC *pDC, CString &str, CRect &rect, BOOL bCalc /* = TRUE */ )
{
	...
	if(!bCalc)
	{
		...
		{
			m_drawmanager.FillEffect(pDC->GetSafeHdc(),
									 m_defStyle.nFillBkgnd,
									 rcTable,
									 m_defStyle.crBkgnd,
									 m_defStyle.crMidBkgnd,
									 m_defStyle.crEndBkgnd,
									 5);
		}
		else if(!m_defStyle.strNameResBk.IsEmpty())
		{
			DrawBackgroundImage(pDC,
								rcTable.left,
								rcTable.top,
								rcTable.Width(),
								rcTable.Height(),
								m_defStyle.strNameResBk);
		}						//if

		m_drawmanager.DrawRectangle(pDC->GetSafeHdc(),
									rcTable,
									m_defStyle.crBorderLight,
									m_defStyle.crBorderDark,
									m_defStyle.nBorderStyle,
			...
			while(!bEndOfRow && (nIndex < str.GetLength()))
			{
				...
				if(nNewCell < nEndRow)
				{
					...
					else
					{
						...
						//Cellspacing
						rcText = rcCell;
						rcText.DeflateRect(m_defStyle.nPaddingLeft,
										   m_defStyle.nPaddingTop,
										   m_defStyle.nPaddingRight,
										   m_defStyle.nPaddingBottom);
						if(m_defStyle.nFillBkgnd >= 0)
						{
							m_drawmanager.FillEffect(pDC->GetSafeHdc(),
													 m_defStyle.nFillBkgnd,
						...
						//Cellpadding
						rcText.DeflateRect(m_defStyle.nMarginLeft,
										   m_defStyle.nMarginTop,
										   m_defStyle.nMarginRight,
										   m_defStyle.nMarginBottom);
	...
	while(i < str.GetLength())
	{
		...
		if(!strText.IsEmpty())
		{
			...
			if(!bCalc)
			{
				...
				{
					//Draw overline
					HPEN	hpenOverline = :: CreatePen(PS_SOLID,
														(m_defStyle.nWeightFont >= FW_BOLD) ? 2 : 1,
														m_defStyle.crText);
			...
			if(!strProperty.IsEmpty())
			{
						...
						if(!bCloseTag)
						{
							...
							while(nIndex < strTag.GetLength())
							{
								...
								if(!strProperty.IsEmpty())
								{
									...
									{
										m_defStyle.nWeightFont = GetStyleFontWeight(strParameter,
																					m_defStyle.nWeightFont);
									}
				...
				else if(strProperty == _T("hr"))
				{
					...
					if(bPercent)
					{
						...
						{
							nWidth = :: MulDiv(m_rect.Width(), nWidth, 100);
						}
					}						//if
					...
					else
					{
						m_drawmanager.DrawLine(pDC->GetSafeHdc(),
											   pt.x,
											   pt.y + m_hline.nHeightLine / 2,
											   pt.x + nWidth,
											   pt.y + m_hline.nHeightLine / 2,
											   csTemp.crText,
											   CPPDrawManager :: PEN_SOLID,
											   csTemp.nBorderWidth);
					}			//if
				...
				else if(strProperty == _T("bmp"))
				{
					...
					if(si.nIdRes || si.nIdDll || si.nHandle || !si.strSrcFile.IsEmpty())
					{
						...
						if(NULL != hBitmap)
						{
							...
							if(si.bPercentWidth)
							{
								sz.cx = :: MulDiv(sz.cx, si.nWidth, 100);
							}
							else
							{
								sz.cx = si.nWidth;
							...
							if(si.bPercentHeight)
							{
								sz.cy = :: MulDiv(sz.cy, si.nHeight, 100);
							}
							else
							{
								sz.cy = si.nHeight;
							...
							if(bCalc)
							{
								m_hline.nHorzAlign = m_defStyle.nHorzAlign;						//Store a last horizontal alignment
																						...
																						CPoint	ptCur =
																							VerticalAlignImage(pt,
																											   szReal.cy,
																											   bCalc);
																						if(sz.cx && sz.cy && !bCalc)
																						{
																							StoreHyperlinkArea(CRect(ptCur.x,
																													 ptCur.y,
																													 ptCur.x + szReal.cx,
																												 ptCur.y +
																													 szReal.cy));

																							if(m_defStyle.nTypeLink !=
																								   LINK_NONE)
																							{
																								if(m_nCurIndexLink ==
																									   m_nHoverIndexLink)
																							...
																							m_drawmanager.DrawBitmap(pDC->m_hDC,
																													 ptCur.x,
																													 ptCur.y,
																													 sz.cx,
																													 sz.cy,
																			...
																			else if(strProperty == _T("icon"))
																			{
																				...
																				if(si.nIdRes || si.nIdDll ||
																				   si.nHandle ||
																				   !si.strSrcFile.IsEmpty())
																				{
																					...
																					if(si.nIdRes)
																					{
																						hIcon = GetIconFromResources(si.nIdRes,
																													 sz);
																					}
																					else if(!si.strSrcFile.IsEmpty())
																					{
																						hIcon = GetIconFromFile(si.strSrcFile,
																												sz);
																					}
																					else if(si.nIdDll)
																					{
																						hIcon = GetIconFromDll(si.nIdDll,
																											   sz,
																											   si.strPathDll);
																					}
																					else if(si.nHandle)
																					...
																					if(NULL != hIcon)
																					{
																						...
																						if(bCalc)
																						{
																							...
																							if(sz.cx && sz.cy && !bCalc)
																							{
																								...
																								m_drawmanager.DrawIcon(pDC->m_hDC,
																													   ptCur.x,
																													   ptCur.y,
																													   sz.cx,
																													   sz.cy,
																				...
																				else if(strProperty == _T("ilst"))
																				{
																					   ...
																					   !si.strSrcFile.IsEmpty())
																					{
																						...
																						{
																							hBitmap = GetBitmapFromDll(si.nIdDll,
																													   si.strPathDll);
																						}
																						else if(si.nHandle)
																						{
																						...
																						if((NULL != hBitmap) && si.cx &&
																						   si.cy)
																						{
																							CSize	szBitmap =
																								m_drawmanager.
																									GetSizeOfBitmap(hBitmap);

																							if(si.bPercentWidth)
																							{
																								si.nWidth = :: MulDiv(si.cx,
																													  si.nWidth,
																													  100);
																							}
																							...
																							if(si.bPercentHeight)
																							{
																								si.nHeight = :: MulDiv(si.cy,
																													   si.nHeight,
																													   100);
																							}

																							CSize	szReal(si.nWidth,
																										   si.nHeight);
																							if(si.nWidth && si.nHeight &&
																							   bShadow)
																							{
																							...
																							DWORD	nMaxCol = szBitmap.
																									cx /
																								si.cx;
																							DWORD	nMaxRow = szBitmap.
																									cy /
																								si.cy;

																							if((si.nIndexImageList
																								   < (int) (nMaxCol *
																						   nMaxRow)) && nMaxCol &&
																							   nMaxRow)
																							{
																								if(bCalc)
																								{
																									m_hline.nHorzAlign =
																										m_defStyle.
																											nHorzAlign; //Store a last horizontal alignment
																									}
																									...
																									if(si.nWidth && si.
																										   ...
																										   && !bCalc)
																									{
																										StoreHyperlinkArea(CRect(ptCur
.
x,
																													 ptCur
.
...
hIconBmp);
																						...
																						else if(NULL != m_ImageList.
																									GetSafeHandle())
																						{
																							if((int) si.nIndexImageList
																								   < m_ImageList.
																								   GetImageCount())
																							{
																								HICON	hIcon =
				...
				//Store a last horizontal alignment
																										}
...
cy,
																					...
																					else if(strProperty == _T("string"))
																					{
																						...
																						{
																							strProperty =
																								SearchPropertyOfTag(strTag,
																													nIndex);
																							if(!strProperty.IsEmpty())
																							{
																								strParameter =
																									GetParameterString(strTag,
																													   nIndex,
																													   _T('='),
																													   _T(" "));
																								if(strProperty ==
																						...
																						if(nIdRes || nIdDll)
																						{
																							...
																							if(!str.IsEmpty())
																							{
																								...
																								if(bCalc)
																								{
																									...
																									CPoint	ptOut =
																										VerticalAlignText(pt,
																														  sz.cy,
																														  bCalc);
																									if(!bCalc)
																									{
																										StoreHyperlinkArea(CRect(ptOut
.
x,
																													 ptOut
.
...
cy));
																						...
																						else if((strProperty == _T(
																									"body")) || (strProperty == _T("h1")) || (strProperty == _T("h2")
																											) || (strProperty == _T("h3")
																													) ||
																														(strProperty == _T("h4")) ||
(strProperty == _T("h5")) ||
...
(strProperty == _T("pre")) ||
(strProperty == _T("big")) ||
(strProperty == _T("small")) ||
(strProperty == _T("sub")) ||
(strProperty == _T("sup")))
																						{
																							m_defStyle.strTag = strProperty;
																							if(StoreRestoreStyle(
																								   bCloseTag))
																							{
																								if(!bCloseTag)
																								{
																									SelectNewHtmlStyle(strProperty,
																													   m_defStyle);
																								}
...
_T(""));
																									...
																									if(m_nCurIndexLink
																										   == m_nHoverIndexLink)
																									{
																										SelectNewHtmlStyle(_T("a:hover"),
																														   m_defStyle);
																									}
																									else
																									{
																										SelectNewHtmlStyle(_T("a:link"),
																														   m_defStyle);
																									}
																								}				//if
...
void CPPHtmlDrawer::SetDefaultCursor()
{
	if(m_hLinkCursor == NULL)	// No cursor handle - load our own
	{
		...
		{	// if not available, load it from winhlp32.exe
			return;
		}
#endif
...
BOOL CPPHtmlDrawer::IsImageWithShadow(_STRUCT_IMAGE &si)
{
	...
	if((dwStyles & IMAGE_EFFECT_MONO_SHADOW) || (dwStyles & IMAGE_EFFECT_GRADIENT_SHADOW))
	{
		return(TRUE);
	}
...
int CPPHtmlDrawer::GetLengthUnit(CString &str, int nDefault, BOOL bFont /* = FALSE */ )
{
	...
	{
		nDefault = nValue * m_pDC->GetTextExtent(_T("x")).cy;
	}
	else if(strUnit == _T("em"))
	{
		nDefault = nValue * m_tm.tmHeight;
	}
	else
	{
		//Gets pixel in inch
		...
		else if(strUnit == _T("cm"))
		{
			nDefault = (int) ((double) nValue / 2.54);
		}
		else if(strUnit == _T("mm"))
		{
			nDefault = (int) ((double) nValue / 25.4);
		}
		else if(strUnit == _T("pt"))
		{
			nDefault = nValue / 72;
		}
		else if(strUnit == _T("pc"))
		{
			nDefault = nValue / 6;
		}
		else
...
int CPPHtmlDrawer::StyleBorderWidth(CString &str, int nDefault)
{
	if(str == _T("thin"))
	{
		nDefault = :: MulDiv(75, nDefault, 100);
	}
	else if(str == _T("thick"))
	{
		nDefault = :: MulDiv(125, nDefault, 100);
	}
	else if(str != _T("medium"))
	{
		nDefault = GetLengthUnit(str, nDefault);
	}
...
int CPPHtmlDrawer::StyleBorder(CString &str, int nDefault)
{
	if((str == _T("none")) || str.IsEmpty())
	{
		nDefault = CPPDrawManager :: PEN_NULL;
	}
	else if(str == _T("solid"))
	{
		nDefault = CPPDrawManager :: PEN_SOLID;
	}
	else if(str == _T("dotted"))
	{
		nDefault = CPPDrawManager :: PEN_DOT;
	}
	else if(str == _T("dashed"))
	{
		nDefault = CPPDrawManager :: PEN_DASH;
	}
	else if(str == _T("double"))
	{
		nDefault = CPPDrawManager :: PEN_DOUBLE;
	}
...
/////////////////////////////////////////////////////////////////
CString CPPHtmlDrawer::GetParameterString(CString &str,
										  int	  &nIndex,
										  TCHAR	  chBeginParam /* = _T(':') */,
										  CString strSeparators /* = _T(";") */
									  )
{
...
CSize CPPHtmlDrawer::AnalyseCellParam(CString &strTag, _STRUCT_CHANGESTYLE &cs)
{
	...
	while(i < strTag.GetLength())
	{
		...
		{
			cs.nMarginLeft = cs.nMarginTop = cs.nMarginRight = cs.nMarginBottom = GetLengthUnit(strParameter,
																								cs.nMarginLeft);
		}
		else if(strProperty == _T("cellspacing"))
		{
			cs.nPaddingLeft = cs.nPaddingTop = cs.nPaddingRight = cs.nPaddingBottom = GetLengthUnit(strParameter,
																									cs.nPaddingLeft);
		}
			...
			switch(str.GetAt(i))
			{
				...
				case _T('s'):
					if(m_szOffsetShadow.cx || m_szOffsetShadow.cy)
					{
						...
						{
							uStyle |= IMAGE_EFFECT_MONO_SHADOW;
						}
					}	//if
					break;
...
void CPPHtmlDrawer::DrawBackgroundImage(CDC *pDC, int nDestX, int nDestY, int nWidth, int nHeight, CString strNameImage)
{
	...
	m_drawmanager.MultipleCopy(pDC->GetSafeHdc(),
							   nDestX,
							   nDestY,
							   nWidth,
							   nHeight,
...
////////////////////////////////////////////////////////////////////
void CPPHtmlDrawer::SetImageShadow(int	   nOffsetX,
								   int	   nOffsetY,
								   BYTE	   nDarkenPercent /* = 50 */,
								   BOOL	   bGradient /* = TRUE */,
								   int	   nDepthX /* = 7 */,
								   int	   nDepthY	/* = 7 */
							   )
