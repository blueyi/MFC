...
HDIB CCeXDib::Create(DWORD dwWidth, DWORD dwHeight, WORD wBitCount)
{
	...
	switch(wBitCount)
	{
		...
		case 32:
			m_wColors = 0;
			break;						// 16,24 or 32 bpp have no color table
...
{
	if((m_hDib == NULL) || (m_wColors == 0) || ((long) dwX < 0) || ((long) dwY < 0) || (dwX >= (DWORD) m_bi.biWidth) ||
	   (dwY >= (DWORD) m_bi.biHeight))
	{
		return;
	}
...
BOOL CCeXDib::WriteBMP(LPCTSTR bmpFileName)
{
	...
	hdr.bfOffBits = (DWORD) sizeof(BITMAPFILEHEADER) + m_bi.biSize + GetPaletteSize();
