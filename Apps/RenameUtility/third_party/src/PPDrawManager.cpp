...
void CPPDrawManager::AlphaBitBlt(HDC	 hDestDC,
								 int	 nDestX,
								 int	 nDestY,
								 DWORD	 dwWidth,
								 DWORD	 dwHeight,
...
HBITMAP CPPDrawManager::CreateImageEffect(HBITMAP	  hBitmap,
										  DWORD		  dwWidth,
										  DWORD		  dwHeight,
										  DWORD		  dwEffect,
										  BOOL		  bUseMask /* = TRUE */,
										  COLORREF	  crMask /* = RGB(255, 0, 255) */,
										  COLORREF	  crMono	/* = RGB(255, 255, 255) */
									  )
{
	...
	COLORREF	*pBits = NULL;
	HBITMAP		hDibBmp = CreateDIBSection(hSrcDC, lpbi, DIB_RGB_COLORS, (void **) &pBits, NULL, NULL);
	...
	for(DWORD pixel = 0; pixel < dwWidth * dwHeight; pixel++, *pBits++)
	{
		...
		{
			if(dwEffect & IMAGE_EFFECT_GRAYEN)
			{
				color = GrayColor(color);
			}

			if(dwEffect & IMAGE_EFFECT_DARKEN)
			{
		...
		if(dwEffect & IMAGE_EFFECT_INVERT)
		{
...
{
	BYTE	nGrayColor = (BYTE)
		((GetRValue(clrColor) * 0.299) + (GetGValue(clrColor) * 0.587) + (GetBValue(clrColor) * 0.114));
...
COLORREF CPPDrawManager::PixelAlpha(COLORREF clrSrc, double src_darken, COLORREF clrDest, double dest_darken)
{
	return
		(
			RGB(GetRValue(clrSrc) * src_darken + GetRValue(clrDest) * dest_darken,
...
void CPPDrawManager::FillGradient(HDC hDC, CRect rect, COLORREF colorStart, COLORREF colorFinish, BOOL bHorz /* = TRUE*/ )
{
...
#ifdef USE_SHADE
void CPPDrawManager::SetShade(CRect		  rect,
							  UINT		  shadeID /* = 0 */,
							  BYTE		  granularity /* = 8 */,
							  BYTE		  coloring /* = 0 */,
							  COLORREF	  hicr /* = 0 */,
...
#endif
void CPPDrawManager::FillEffect(HDC			hDC,
								DWORD		dwEffect,
								LPCRECT		lpRect,
								COLORREF	clrBegin,
								COLORREF	clrMid /* = 0 */,
...
*/
		...
		case EFFECT_HCGRADIENT:
			FillGradient(hDC,
						 CRect(rect.left, rect.top, rect.left + rect.Width() / 2, rect.bottom),
						 clrBegin,
						 clrEnd,
						 TRUE);
			FillGradient(hDC,
						 CRect(rect.left + rect.Width() / 2, rect.top, rect.right, rect.bottom),
						 clrEnd,
						 clrBegin,
						 TRUE);
			break;
		...
		case EFFECT_VCGRADIENT:
			FillGradient(hDC,
						 CRect(rect.left, rect.top, rect.right, rect.top + rect.Height() / 2),
						 clrBegin,
						 clrEnd,
						 FALSE);
			FillGradient(hDC,
						 CRect(rect.left, rect.top + rect.Height() / 2, rect.right, rect.bottom),
						 clrEnd,
						 clrBegin,
						 FALSE);
			break;
		...
		case EFFECT_3HGRADIENT:
			FillGradient(hDC,
						 ...
						 clrMid,
						 TRUE);
			FillGradient(hDC, CRect(rect.left + rect.Width() / 2, rect.top, rect.right, rect.bottom), clrMid, clrEnd, TRUE);
			break;
...
void CPPDrawManager::MultipleCopy(HDC	  hDestDC,
								  int	  nDestX,
								  int	  nDestY,
								  DWORD	  dwDestWidth,
								  DWORD	  dwDestHeight,
...
void CPPDrawManager::DrawBitmap(HDC			hDC,
								int			x,
								int			y,
								DWORD		dwWidth,
								DWORD		dwHeight,
							...
							)
{
	...
	if(bShadow)
	{
		if(dwEffect & IMAGE_EFFECT_SHADOW)
		{
			...
			HBITMAP hShadowBmp = CreateImageEffect(hBitmapTemp,
												   dwWidth,
												   dwHeight,
												   IMAGE_EFFECT_MASK,
												   bUseMask,
												   crMask,
												   InvertColor(clrShadow));
			DrawShadow(hDC,
					   ptShadow.x,
					   ptShadow.y,
					   dwWidth,
					   dwHeight,
					   hShadowBmp,
					   dwEffect & IMAGE_EFFECT_GRADIENT_SHADOW,
					   dwCxDepth,
					   dwCyDepth);
...
void CPPDrawManager::DrawIcon(HDC		  hDC,
							  int		  x,
							  int		  y,
							  DWORD		  dwWidth,
							  DWORD		  dwHeight,
						  ...
						  )
{
	...
	{
		hIcon = StretchIcon(hSrcIcon, dwWidth, dwHeight);
	}
	...
	if(bShadow)
	{
		if(dwEffect & IMAGE_EFFECT_SHADOW)
		{
			...
			HBITMAP hShadowBmp = CreateImageEffect(csOriginal.hbmMask,
												   dwWidth,
												   dwHeight,
												   IMAGE_EFFECT_MASK,
												   TRUE,
...
void CPPDrawManager::DrawShadow(HDC		hDestDC,
								int		nDestX,
								int		nDestY,
								DWORD	dwWidth,
								DWORD	dwHeight,
								...
								DWORD	dwDepthX /* = PPDRAWMANAGER_SHADOW_YOFFSET */,
								DWORD	dwDepthY	/* = PPDRAWMANAGER_SHADOW_XOFFSET */
							)
{
...
void CPPDrawManager::MaskToDepth(HDC	 hDC,
								 DWORD	 dwWidth,
								 DWORD	 dwHeight,
								 HBITMAP hMask,
								 double	 *pDepth,
								 BOOL	 bGradient /* = FALSE */,
								 DWORD	 dwDepthX /* = PPDRAWMANAGER_CXSHADOW */,
								 DWORD	 dwDepthY	/* = PPDRAWMANAGER_CYSHADOW */
							 )
{
	HDC hSrcDC = :: CreateCompatibleDC(hDC);
	if(NULL == hSrcDC)
...
{
	double *const	pfBuff1 = new double[(ImageWidth + KerWidth - 1) * (ImageHeight + KerHeight - 1)];
	double *const	pfBuff2 = new double[(ImageWidth + KerWidth - 1) * (ImageHeight + KerHeight - 1)];
...
void CPPDrawManager::DrawRectangle(HDC		   hDC,
								   LPRECT	   lpRect,
								   COLORREF	   crLight,
								   COLORREF	   crDark,
								   int		   nStyle /* = PEN_SOLID */,
								   int		   nSize	/* = 1 */
							   )
{
							   ...
							   )
{
	...
	switch(nStyle)
	{
		...
		case PEN_DASHDOT:
			nSysPenStyle = PS_DASHDOT;
			break;

		case PEN_DASHDOTDOT:
			nSysPenStyle = PS_DASHDOTDOT;
			break;
