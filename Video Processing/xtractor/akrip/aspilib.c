...
int loadAspi( void );
int loadScsipt( void );
DWORD SetAPI( DWORD dwInterface );
void deinitLibrary( void );
void deinitAspi( void );
void deinitScsipt( void );
...
DWORD SetAPI( DWORD dwInterface )
{
  if ( dwInterface == APIC_AUTO )
  {
#ifdef _DEBUG
    dbprintf( "akrip32: SetAPI( APIC_AUTO )" );
#endif
    if ( loadAspi() || loadScsipt() )
      return SS_COMP;
    else
      return SS_ERR;
  }
  
  if ( dwInterface == APIC_SCSIPT )
  {
#ifdef _DEBUG
    dbprintf( "akrip32: SetAPI( APIC_SCSIPT )" );
#endif
    if ( loadScsipt() )
      return SS_COMP;
    else
  ...
  if ( dwInterface == APIC_ASPI )
  {
#ifdef _DEBUG
    dbprintf( "akrip32: SetAPI( APIC_ASPI )" );
#endif
    if ( loadAspi() )
      return SS_COMP;
    else
  ...
  alErrCode = ALERR_INVALIDADAPTER;
  return SS_ERR;
}
...
#ifdef _DEBUG
    dbprintf( "akrip32: loadAspi()" );
#endif
  if ( dwAPI == APIC_SCSIPT )
  {
    deinitScsipt();
  }
...
#ifdef _DEBUG
    dbprintf( "akrip32: loadAspi() successful" );
#endif
   return 1;
}
...
#ifdef _DEBUG
      dwErr = GetLastError();
      dbprintf( "Unable to initialize SCSIPT" );
      dbprintf( "akrip32: GetLastError() -> %d (%04X)", dwErr, dwErr );
#endif
      dwAPI = APIC_NONE;
...
#ifdef _DEBUG
    dbprintf( "akrip32: loadScsipt() successful" );
#endif
   return 1;
}
...
void deinitLibrary( void )
{
  ...
  {
#ifdef _DEBUG
    dbprintf( "akrip32: deinitLibrary( APIC_NONE )" );
#endif
    return;
  } 
  else if ( dwAPI == APIC_ASPI )
  {
#ifdef _DEBUG
    dbprintf( "akrip32: deinitLibrary( APIC_ASPI )" );
#endif
    deinitAspi();
  }
  else if ( dwAPI == APIC_SCSIPT )
  {
#ifdef _DEBUG
    dbprintf( "akrip32: deinitLibrary( APIC_SCSIPT )" );
#endif
    deinitScsipt();
  }
...
#ifdef _DEBUG
  dbprintf( "ReadCDAudioLBAEx(): start=%d, num=%d, over start=%d, over num=%d",
            t->startFrame, t->numFrames, tOver->startFrame, tOver->numFrames );
#endif
...
#ifdef _DEBUG
	  dbprintf( "zeroing out overlap buffer, tOver->startFrame == %d, t->startFrame == %d, j == %d", tOver->startFrame, t->startFrame, j );
#endif
	}
    }
