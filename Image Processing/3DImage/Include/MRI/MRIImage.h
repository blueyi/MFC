...
class __declspec(dllexport) CMRIImage
{
public:
	float GetPTPos();
	...
	void Transformation( double dNewCenter[3], double dNewPhaseDir[3], double dNewFreqDir[3] );
...
private:
	...
	void ComputeOrientChar(CT_STRUCT_IMAGE *pImage);
	void ComputeOrientChar(MR_STRUCT_IMAGE *pImage);
	...
	bool CheckOrientation( double dOrient[3] );
