...
{    
    ...
    {
        _tcscpy(pTTT->szText, _T("Check Box Control\rCheck this if you want\ntooltips to be bold"));
        return TRUE;
    }
...
void CToolTipExDlg::OnButtonColorBackground() 
{
	...
	CColorDialog dlgColor(m_clrBackground,CC_FULLOPEN,NULL);
	if(dlgColor.DoModal()==IDOK)
	{
		m_clrBackground=dlgColor.GetColor();
...
void CToolTipExDlg::OnButtonTextFontColor() 
{
	...
	dlgFont.m_cf.rgbColors=m_clrText;
...
void CToolTipExDlg::OnCheck1() 
{
	...
	VERIFY(m_font.GetLogFont(&lf));
	VERIFY(m_font.DeleteObject());
	lf.lfWeight=(m_bBoldTips ? FW_BOLD : FW_NORMAL);
	VERIFY(m_font.CreateFontIndirect(&lf));
	m_toolTip.SetFont(&m_font);
}
