...
void CFireWnd::SetPalette(RGBTRIPLE *rgbPalette)
{
	if (m_Palette.m_hObject != NULL)
	...
	//如果rgbPalette为空，则使用默认调色板颜色值，jingzhou xu
	if(rgbPalette != NULL)
	{
		for (int i = 0; i < 256; i++)
		{
			lpLogPal->palPalEntry[i].peRed = m_rgbPalette[i].rgbRed = rgbPalette[i].rgbtRed;
			lpLogPal->palPalEntry[i].peGreen = m_rgbPalette[i].rgbGreen = rgbPalette[i].rgbtGreen;
			lpLogPal->palPalEntry[i].peBlue = m_rgbPalette[i].rgbBlue = rgbPalette[i].rgbtBlue;
			lpLogPal->palPalEntry[i].peFlags = NULL;
		}
	}
	else
	{
		RGBQUAD*    pColorTab = rgbStd256;
		for (int i = 0; i < 256; i++)
		{
			lpLogPal->palPalEntry[i].peRed = m_rgbPalette[i].rgbRed = pColorTab[i].rgbRed;
			lpLogPal->palPalEntry[i].peGreen = m_rgbPalette[i].rgbGreen = pColorTab[i].rgbGreen;
			lpLogPal->palPalEntry[i].peBlue = m_rgbPalette[i].rgbBlue = pColorTab[i].rgbBlue;
			lpLogPal->palPalEntry[i].peFlags = NULL;
		}
	}
	...
	SetDIBColorTable(m_pMemDC->m_hDC, 0, 255, m_rgbPalette);
...
void CFireWnd::RenderFlame()
{
	...
	if (m_nSmoothness > 0)
	{
		...
		for (x = xStart; x < xEnd; x++)
		{
			...
			m_Fire[x] = (BYTE)(val / (2*m_nSmoothness+1));
		}
	}
}
...
void CFireWnd::PreSubclassWindow() 
{
