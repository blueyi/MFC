...
void CMyTabCtrl::CreateEdit(LPCTSTR sCaption, int nID, int iTab, UINT uLocation, int iX, int iY, int iLen)
{
	CRect rc;
	GetClientRect(&rc);//GetWindowRect
	int x = rc.left + MARGIN_LEFT;
	...
	pEdit->Create(WS_CHILD|WS_VISIBLE|WS_TABSTOP|WS_BORDER|ES_AUTOHSCROLL,CRect(x,y,x+sz.cx,y+sz.cy),this,nID);
	pEdit->SetFont(GetFont());
	if(iTab == 0){pEdit->ShowWindow(SW_SHOW);}else{pEdit->ShowWindow(SW_HIDE);}
	pEdit->SetLimitText(MAX_PATH);
